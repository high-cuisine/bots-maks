...
const receiver = new Api.InputPeerUser({
    userId: '1042650482',
    accessHash: "-4491671000857606737"
});
...
(async () => {
    ...
    client.addEventHandler(async (event) => {
        ...
        if (checkWords(triggerWords, message.message)) {
            try {
                const cha = await client.getEntity(4256035475);
        
                if (Boolean(message.peerId.channelId)) {
                        const link = `https://t.me/c/${message.peerId.channelId.value}/${message.id}`
                        await client.sendMessage(cha, {
                            message: `${link}`
                        })
                } else {
                    
                    const link = `https://t.me/c/${message.peerId.chatId.value}/${message.id}`
